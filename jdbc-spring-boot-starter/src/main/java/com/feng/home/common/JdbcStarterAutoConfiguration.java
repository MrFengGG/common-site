...
public class JdbcStarterAutoConfiguration {
    ...
    DataSource shardingDataSource() throws Exception {
        List<ModelMapping> modelMappings = getModelMappings(dataSourceConfig.getBasePackage());
        //创建分库分表规则
        ShardingRuleConfiguration shardingRuleConfiguration = new ShardingRuleConfiguration();
        modelMappings.stream().filter(ModelMapping::sharding).collect(Collectors.toList()).forEach(modelMapping -> {
    ...
    private List<ModelMapping> getModelMappings(String basePackage) throws Exception {
        PackageScanner scanner = new SamplePackageScanner();
        try {
            List<ModelMapping> modelMappings = scanner.scanAndGetAnnotations(ModelMapping.class, basePackage);
